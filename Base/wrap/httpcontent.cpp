...
{
	inflateEnd(z);
	*zlib_init = Wrap::CHttpDownload::ZLIB_UNINIT;
	return result;
}

static int inflate_stream(Wrap::CHttpDownload* k)
{
	int allow_restart = 1;
...
int Http_unencode_deflate_write(Wrap::CHttpDownload* k, unsigned char* httbBuf, unsigned int httpBufLen)
{
	...
	/* Initialize zlib? */
	if (k->zlib_init == Wrap::CHttpDownload::ZLIB_UNINIT) {
		...
		k->zlib_init = Wrap::CHttpDownload::ZLIB_INIT;
	}
...
int Http_unencode_gzip_write(Wrap::CHttpDownload* k, unsigned char* httbBuf, unsigned int httpBufLen)
{
	...
	if (k->zlib_init == Wrap::CHttpDownload::ZLIB_UNINIT) {
		...
		if (strcmp(zlibVersion(), "1.2.0.4") >= 0) {
			...
			k->zlib_init = Wrap::CHttpDownload::ZLIB_INIT_GZIP; /* Transparent gzip decompress state */
		}
		else {
			...
			k->zlib_init = Wrap::CHttpDownload::ZLIB_INIT;   /* Initial call state */
		}
	}

	if (k->zlib_init == Wrap::CHttpDownload::ZLIB_INIT_GZIP) {
	...
	case Wrap::CHttpDownload::ZLIB_INIT:
	{
		/* Initial call state */
		unsigned int hlen;
		...
		case GZIP_OK:
			...
			k->zlib_init = Wrap::CHttpDownload::ZLIB_GZIP_INFLATING; /* Inflating stream state */
			break;
		...
		case GZIP_UNDERFLOW:
			...
			k->zlib_init = Wrap::CHttpDownload::ZLIB_GZIP_HEADER;   /* Need more gzip header data state */
	...
	case Wrap::CHttpDownload::ZLIB_GZIP_HEADER:
	{
	...
	case Wrap::CHttpDownload::ZLIB_GZIP_INFLATING:
	default:
