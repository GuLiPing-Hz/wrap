...
#define qValidChar(u)        ((u) ? (u) : static_cast<unsigned short>(QChar::ReplacementCharacter))
...
typedef struct {
	unsigned char        tblBegin;
	unsigned char        tblEnd;
	unsigned short        tblOffset;
	unsigned short        algOffset;
} indexTbl_t;

static unsigned int qt_Gb18030ToUnicode(const unsigned char *gbstr, int& len);
static int qt_UnicodeToGb18030(unsigned int unicode, unsigned char *gbchar);
int qt_UnicodeToGbk(unsigned int unicode, unsigned char *gbchar);

//需要自己释放 返回的指针,失败返回NULL
unsigned short* QGb18030Codec::convertToUnicode(const char * chars, int len) const
{
	if(!chars || len == 0)
		return NULL;
...
/*
	...
	int unicodeLen = 0;
	unsigned short *const resultData = reinterpret_cast<unsigned short*>(result);
	...
	for (int i = 0; i < len; i++) {
		...
		case 1:
			...
			if (Is2ndByteIn2Bytes(ch)) {
				buf[1] = ch;
				int clen = 2;
				unsigned int u = qt_Gb18030ToUnicode(buf, clen);
				if (clen == 2) {
					resultData[unicodeLen] = qValidChar(static_cast<unsigned short>(u));
					++unicodeLen;
				} else {
					resultData[unicodeLen] = replacement;
					++unicodeLen;
...
//需要自己释放 返回的指针,失败返回NULL
char* QGb18030Codec::convertFromUnicode(const unsigned short *uc, int len) const
{
	if(!uc || len==0)
		return NULL;
...
/*
	...
	for (int i = 0; i < len; i++) {
		...
		unsigned char buf[4];
		if (high >= 0) {
			if (/*uc[i].isLowSurrogate()*/QChar::isLowSurrogate((unsigned short)uc[i])) {
				// valid surrogate pair
				++i;
				unsigned int u = QChar::surrogateToUcs4(high, uc[i]);
		...
		if (IsLatin(ch)) {
			// ASCII
			*cursor++ = ch;
		} else if (/*uc[i].isHighSurrogate()*/QChar::isHighSurrogate((unsigned short)uc[i])) {
	...
	//rstr.resize(cursor - (unsigned char*)rstr.constData());
...
//需要自己释放 返回的指针,失败返回NULL
unsigned short* QGbkCodec::convertToUnicode(const char *chars, int len) const
{
	if(!chars || len==0)
		return NULL;
	unsigned char buf[2];
...
//需要自己释放 返回的指针,失败返回NULL
char* QGbkCodec::convertFromUnicode(const unsigned short *uc, int len) const
{
	if(!uc || len==0)
		return NULL;
	char replacement = '?';
	...
	for (int i = 0; i < len; i++) {
		...
		if (QChar::row(ch)/*ch.row()*/ == 0x00 && QChar::cell(ch)/*ch.cell()*/ < 0x80) {
			// ASCII
			*cursor++ = QChar::cell(ch);
...
//需要自己释放 返回的指针,失败返回NULL
unsigned short* QGb2312Codec::convertToUnicode(const char *chars, int len) const
{
	if(!chars || len)
		return NULL;
	unsigned char buf[2];
...
//需要自己释放 返回的指针,失败返回NULL
char* QGb2312Codec::convertFromUnicode(const unsigned short *uc, int len) const
{
	char replacement = '?';
/*
	if (state) {
...
// 	rstr.resize(rlen);
	...
	for (int i = 0; i < len; i++) {
		...
		if (/*ch.row()*/QChar::row(ch) == 0x00 && /*ch.cell()*/QChar::cell(ch) < 0x80) {
			// ASCII
			*cursor++ = QChar::cell(ch);//ch.cell();
		} else if ((qt_UnicodeToGbk(ch, buf) == 2) &&
...
static unsigned short const ucs_to_gb18030[28839] = {
...
static unsigned short const gb18030_2byte_to_ucs[22046] = {
	/* GB 0x8140..0x817E */
...
static unsigned short const gb18030_4byte_to_ucs[6793] = {
...
static inline unsigned int gb4lin_to_gb(unsigned int gb4lin) {
...
static unsigned int qt_Gb18030ToUnicode(const unsigned char *gbstr, int& len) {
	/* Returns Unicode. */
	unsigned int    uni;
	unsigned char   first = *gbstr;

	if (IsLatin(first)) {
		len = 1;
		uni = (unsigned int)first;
	}
	else if (Is1stByte(first) && len >= 2) {
		unsigned char        second = gbstr[1];
		...
		if (Is2ndByteIn2Bytes(second)) {
			...
			if (IsUDA1(first, second))
				...
				uni = (unsigned int)gb18030_2byte_to_ucs[i];
			}
		}
		else if (Is2ndByteIn4Bytes(second) && len >= 4) {
			unsigned char   third  = gbstr[2],
				fourth = gbstr[3];
			...
			if (Is3rdByte(third) && Is4thByte(fourth)) {
				...
				if (gb4lin <= 0x99FB) {
					...
					if ((unsigned char)(gb4lin & 0xFF) >= g2u.tblBegin &&
						(unsigned char)(gb4lin & 0xFF) <= g2u.tblEnd) {

							uni = (unsigned int)gb18030_4byte_to_ucs[gb4lin - g2u.tblOffset];
					}
					else {
...
int qt_UnicodeToGb18030(unsigned int uni, unsigned char *gbchar) {
	/* Returns the bytesize of the GB18030 character. */
	unsigned int        gb, gb4lin;
	indexTbl_t        u2g;

	if (IsLatin(uni)) {
		*gbchar = (unsigned char)uni;
		return 1;
	}
	else if (uni <= 0xD7FF || InRange(uni, 0xE766, 0xFFFF)) {
		u2g = ucs_to_gb18030_index[uni >> 8];

		if ((unsigned char)(uni & 0xFF) >= u2g.tblBegin && (unsigned char)(uni & 0xFF) <= u2g.tblEnd) {
	...
	if (gb <= 0xFFFF) {
		gbchar[0] = (unsigned char)((gb >> 8) & 0xFF);
		gbchar[1] = (unsigned char)(gb & 0xFF);
		return 2;
	} else {
		gbchar[0] = (unsigned char)((gb >> 24) & 0xFF);
		gbchar[1] = (unsigned char)((gb >> 16) & 0xFF);
		gbchar[2] = (unsigned char)((gb >> 8) & 0xFF);
		gbchar[3] = (unsigned char)(gb & 0xFF);
		return 4;
	}
}


int qt_UnicodeToGbk(unsigned int uni, unsigned char *gbchar) {
